...
export async function getProductById(id: string) {
  ...
  // Forzamos las cabeceras de navegador directo desde el Server Component
  const secureHeaders = {
    "Accept": "application/json",
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
  };

  // LLAMADA DIRECTA: Nos saltamos apiCall y nos saltamos el proxy
  ...
  if (!res.ok) {
    throw new Error(`Error directo de FakeStore. Status: ${res.status}`);
  }
